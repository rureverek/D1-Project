...
while time<=1
    ...
    %Change duty cycle based on error (PID Controller)
     if(mod(time,0.01)<timeStep)   %Change every 0.01 seconds      
         error=Vout-Vout_target;   
         diferror = (error-errorprev)/0.01;%derivative (slope) of e(t)
         sumerror = sumerror + error*0.01; %integral of e(t')dt' in [0 t]
         
          kP=0.007;         
          dutyCycle=dutyCycle-error*kP;   %proportional term
          kI = 0.016;
          dutyCycle=dutyCycle-sumerror*kI;%integral term
          kD = 0.0005;
          dutyCycle=dutyCycle-diferror*kD;%derivative term
...
%hold all, plot(time_store,dutyCycle_store);
xlabel('Time [s]');
